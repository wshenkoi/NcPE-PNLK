...
def run(args, trainset, testset, action):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        running_loss, running_info, avg_overlap = action.train_1(model, trainloader, optimizer, args.device)
        val_loss, val_info = action.eval_1(model, testloader, args.device)

        is_best = val_loss < min_loss
        min_loss = min(val_loss, min_loss)

        LOGGER.logger.info('epoch, %04d, overlap, %f, %f, %f, %f, %f', epoch + 1, avg_overlap, running_loss, val_loss, running_info, val_info)
        snap = {'epoch': epoch + 1,
...
class Action:
    ...
    def train_1(self, model, trainloader, optimizer, device):
        ...
        for i, data in enumerate(trainloader):
            loss, loss_g, overlap = self.compute_loss(model, data, device)
            ...
            info = "current g_loss: {:.3f}(ave_gloss: {:.3f})".format(loss_g, ave_gloss)
            t.set_description(desc=info)
            t.update()

        ave_vloss = float(vloss)/count
        avg_overlap = float(overlaps)/count
        
        t.close()
        
        return ave_vloss, ave_gloss, avg_overlap
    ...
    def eval_1(self, model, testloader, device):
        ...
        with torch.no_grad():
            for i, data in enumerate(testloader):
                loss, loss_g, _ = self.compute_loss(model, data, device)
...
#                ptlk.data.transforms.Mesh2Points(),\
        ...
        traindata = ptlk.data.getdata.ModelNet(args.dataset_path, train=1,\
                                                transform=transform, classinfo=cinfo, crop = args.crop)
        testdata = ptlk.data.getdata.ModelNet(args.dataset_path, train=0,\
                                                transform=transform, classinfo=cinfo, crop = args.crop)

        mag_randomly = True
        trainset = ptlk.data.getdata.CADset4tracking(traindata,\
                        ptlk.data.transforms.RandomTransformSE3(args.mag, mag_randomly))
        testset = ptlk.data.getdata.CADset4tracking(testdata,\
                        ptlk.data.transforms.RandomTransformSE3(args.mag, mag_randomly))
